...
def positional_encoding_2d(row, col, d_model):
  ...
  col_pos = np.repeat(np.expand_dims(np.arange(col), 0), row,
                      axis=0).reshape(-1, 1)
  angle_rads_row = get_angles(row_pos,
                              np.arange(d_model // 2)[np.newaxis, :],
                              d_model // 2)
  ...
  pos_encoding = np.concatenate([angle_rads_row, angle_rads_col],
                                axis=1)[np.newaxis, ...]
...
class Transformer(tf.keras.Model):
  ...
  def __init__(self,
    ...
    self.encoder = Encoder(num_layers, d_model, num_heads, dff, row_size,
                           col_size, rate)
...
def evaluate(image):
  ...
  for i in range(100):
    ...
    predictions, attention_weights = transformer(img_tensor_val, output, False,
                                                 dec_mask)
...
uploaded_images = st.file_uploader("Choose multiple images...",
                                   type=["jpg", "png"],
